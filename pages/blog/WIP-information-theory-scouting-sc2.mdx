...
- Apply Shannon Entropy to builds by calculating frequency of appearance
    - Naive calculation, room for improvement by calculating frequency based on time
        - Accounts for scenarios where common buildings are created at odd times. I.e. rushing a base
...
- Entropy of army. Entropy per unit, or per cost?
...
After removing these buildings I calculated building frequencies on a per matchup basis to try and increase accuracy and
...
RO4 games between Reynor and Maru and RO8 games between TY and PartinG and checked the values
to see if they made any sort of sense.
...
Right now I haven't charted these values so I have nothing to show,
**<span style={{ textDecoration: 'underline' }}>but I plan to visualize this soon</span>**.

An interesting unexpected result that I didn't think about was Terran add-ons being counted as
a unique building + add-on pair (E.g. `StarportReactor`). Since these combinations are
relatively rare compared to other buildings they also tend to have quite high entropy, which
makes sense when you think about it because add-on choice is actually quite important and
can signal different builds!

## Improvements and Future Applications

The main issue I see with the current methdology is naively calculating frequencies based on
...
each building which include the required tech buildings, and separately recording buildings from
between when new tech buildings were created. While probably more 'correct', this seems quite a complex solution though.

On the other side of things, an obvious application is units/army. The issue with this is that
...
when players suddenly produce many units, which might mean you want to scout them and see if they
are planning an attack.

This is similar to simply batching by supply or army value, but the difference is that we are
...
Another possible application of entropy to army is a measurement such as entropy per unit/resource/supply.
...
I'm excited to see where this goes since I think this idea is extremely flexible and applicable
to many different areas of both SC2 and other games.
