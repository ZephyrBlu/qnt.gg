...
export const postDetails = (preview = false, postPath = '') => (
    <PostDetails
        ...
        blurb="Quantitative Finance and Gaming are more similar than you might think"
        postedAt="WIP"
    />
);
...
At first glance quantitative finance seems completely unrelated to gaming, but after
...
linearly or cyclically over time.
...
many games have large amounts of easily accessible, fairly rich game data.

There is one obvious downside of game data though. Financial data is very straightforward.
You have a price, it goes up and down. End of story. Games are not so straightforwards.
...
and select the variables that best model those questions.

Though you could actually say that this is a strength of game data since we have far more variables
to play with!
